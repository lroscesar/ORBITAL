...
export function Cadastro({ onGoLogin }: { onGoLogin: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // 🔑 CORREÇÃO: cria a conta de verdade no Supabase (isto dispara o e-mail de confirmação)
    const { error } = await supabase.auth.signUp({
      email,
      password: senha,
      options: {
        data: { nome },                          // guarda o nome no perfil (user_metadata)
        emailRedirectTo: window.location.origin, // link do e-mail volta pra sua URL (Vercel)
      },
    })
...
export function Login({ onGoRegister, onGoRecover }: { onGoRegister: () => void; onGoRecover: () => void }) {
  const [email, setEmail] = useState("")
  const [senha, setSenha] = useState("")
  const [loading, setLoading] = useState(false)
  const [err, setErr] = useState("")
